...
import cast.android.ui.viewmodel.FakeDownloadRepository
...
class EpisodeRepositoryImplTest {
    ...
    private fun repo(vararg recent: cast.api.EpisodeDetailDto) =
        EpisodeRepositoryImpl(FakeCastApiService(recent = recent.toList()), FakeDownloadRepository())

    @Test
    fun `cachedRecentEpisodes is null before any fetch`() = runTest {
        val repo = repo(episode("1"))
        assertNull(repo.cachedRecentEpisodes())
    }

    @Test
    fun `cachedRecentEpisodes returns the last fetched list`() = runTest {
        val repo = repo(episode("1"), episode("2"))
