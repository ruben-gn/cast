...
import cast.android.ui.nav.DownloadedEpisodes
...
fun SettingsScreen(navController: NavHostController) {
    ...
    Column(
        ...
        Row(
            modifier = Modifier
                .fillMaxWidth()
                .clickable { navController.navigate(DownloadedEpisodes) },
            horizontalArrangement = Arrangement.SpaceBetween,
            verticalAlignment = Alignment.CenterVertically,
        ) {
            Row(
                ...
                Text("Downloaded episodes", style = MaterialTheme.typography.bodyLarge)
            }
        ...
        // Only CI builds carry a "<commits>-<sha>" name, and only those have a release to link to.
        val uriHandler = LocalUriHandler.current
        val isCiBuild = BuildConfig.VERSION_NAME.contains('-')
        Text(
            "Build ${BuildConfig.VERSION_NAME}",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
            modifier = if (isCiBuild) {
                Modifier.clickable {
                    uriHandler.openUri(
                        "https://github.com/ruben-gn/cast/releases/tag/v${BuildConfig.VERSION_NAME}",
                    )
                }
            } else {
                Modifier
